...
def plot_SP(X_df, preds, optimal_threshold, file, hyperparameter_string):
    
    # check if doublethresholding was used
    if type(optimal_threshold) is tuple:
        lower_threshold, upper_threshold = optimal_threshold
    else: 
        lower_threshold, upper_threshold = -optimal_threshold, optimal_threshold
...
def plot_predictions(X_dfs, predictions, optimal_threshold, dfs_files, current_method, hyperparameter_string, which_stations = None):
    ...
    for station in which_stations:
        ...
        if current_method == "SingleThresholdSP" or current_method == "DoubleThresholdSP":
            plot_SP(X_df, preds, optimal_threshold, file, hyperparameter_string)