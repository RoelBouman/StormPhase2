...
# @jit(nopython=True)
def calculate_bootstrap_stats(y_dfs, y_scores_dfs, y_pred_dfs, label_filters_for_all_cutoffs, beta,  bootstrap_iterations=10000):
    
    n_cutoffs = len(label_filters_for_all_cutoffs[0])
    n_dfs = len(y_dfs)
    ...
    for i in range(bootstrap_iterations):
        ...
        y_dfs_bootstrapped = [y_dfs[j] for j in bootstrap_indices]
        #y_scores_dfs_bootstrapped = [y_scores_dfs[j] for j in bootstrap_indices]
        y_pred_dfs_bootstrapped = [y_pred_dfs[j] for j in bootstrap_indices]
        label_filters_for_all_cutoffs_bootstrapped = [label_filters_for_all_cutoffs[j] for j in bootstrap_indices]
        
        
        PRFAUC_table_per_iteration[:,:,i] = calculate_PRFAUC_table(y_dfs_bootstrapped, y_pred_dfs_bootstrapped, label_filters_for_all_cutoffs_bootstrapped, beta)
        metric_per_iteration[i] = cutoff_averaged_f_beta(y_dfs_bootstrapped, y_pred_dfs_bootstrapped, label_filters_for_all_cutoffs_bootstrapped, beta)
